      ...
      <header class="page-hero">
        <div>
          ...
          <h1 class="page-hero__title">
            {{ isEditing ? 'Modifier un beneficiaire' : 'Nouveau beneficiaire' }}
          </h1>
          <p class="page-hero__copy">
            Organisez les informations personnelles, de contact et de programme dans une fiche
            claire et responsive.
          </p>
        </div>
        ...
        <form class="beneficiary-form" @submit.prevent="submitForm">
          <section class="form-section">
            <div class="form-section__header">
              ...
              <p>Identite, code unique, photo et elements de base du profil.</p>
            </div>
            <div class="form-grid form-grid--three">
              <div class="field-block">
                <label class="field-label" for="unique_code">Code unique</label
                ><input
                  id="unique_code"
                  v-model="form.unique_code"
                  type="text"
                  class="field-input"
                  :class="{ 'is-invalid': errors.unique_code }"
                  placeholder="Ex: BNF-2026-001"
                />
                <p v-if="errors.unique_code" class="field-error">{{ errors.unique_code }}</p>
              </div>
              <div class="field-block">
                <label class="field-label" for="first_name"
                  >Prenom <span class="required-mark">*</span></label
                ><input
                  id="first_name"
              ...
              <div class="field-block">
                <label class="field-label" for="last_name"
                  >Nom <span class="required-mark">*</span></label
                ><input
                  id="last_name"
              ...
              <div class="field-block">
                ...
                />
                <p v-if="computedAge !== null" class="field-help">
                  Age estime: {{ computedAge }} ans
                </p>
                <p v-if="errors.date_of_birth" class="field-error">{{ errors.date_of_birth }}</p>
              </div>
              <div class="field-block">
                <label class="checkbox-row"
                  ><input v-model="form.is_disabled" type="checkbox" class="checkbox-input" /><span
                    >Beneficiaire en situation de handicap</span
                  ></label
                >
              </div>
            </div>
          ...
          <section class="form-section">
            <div class="form-section__header">
              <h2>Contact</h2>
              <p>Coordonnees actuelles et lieu de residence.</p>
            </div>
            <div class="form-grid">
              <div class="field-block">
              ...
              <div class="field-block">
                <label class="field-label" for="current_country">Pays actuel</label
                ><input
                  id="current_country"
                  v-model="form.current_country"
                  type="text"
                  class="field-input"
                  :class="{ 'is-invalid': errors.current_country }"
                  placeholder="Pays de residence"
                />
                <p v-if="errors.current_country" class="field-error">
                  {{ errors.current_country }}
                </p>
              </div>
              <div class="field-block">
                <label class="field-label" for="current_city">Ville actuelle</label
                ><input
                  id="current_city"
                  v-model="form.current_city"
                  type="text"
                  class="field-input"
                  :class="{ 'is-invalid': errors.current_city }"
                  placeholder="Ville de residence"
                />
                <p v-if="errors.current_city" class="field-error">{{ errors.current_city }}</p>
              </div>
            </div>
          </section>
          ...
          <section class="form-section">
            <div class="form-section__header">
              ...
              <p>Type de beneficiaire, niveau initial, vulnerabilite et suivi.</p>
            </div>
            <div class="form-grid">
              <div class="field-block">
              ...
              <div class="field-block">
                <label class="field-label" for="entry_date">Date d'entree</label
                ><input
                  id="entry_date"
                ...
                />
                <p v-if="errors.entry_date" class="field-error">{{ errors.entry_date }}</p>
              </div>
              <div class="field-block">
                <label class="field-label" for="exit_date">Date de sortie</label
                ><input
                  id="exit_date"
                  v-model="form.exit_date"
                  type="date"
                  class="field-input"
                  :class="{ 'is-invalid': errors.exit_date }"
                  :disabled="form.type !== 'alumni'"
                />
                <p class="field-help">
                  {{
                    form.type === 'alumni'
                      ? 'Champ actif pour les alumni.'
                      : 'Disponible uniquement pour les alumni.'
                  }}
                </p>
                <p v-if="errors.exit_date" class="field-error">{{ errors.exit_date }}</p>
              </div>
              <div class="field-block">
                <label class="field-label" for="vulnerability_type">Type de vulnerabilite</label
                ><input
                  id="vulnerability_type"
                  v-model="form.vulnerability_type"
                  type="text"
                  class="field-input"
                  :class="{ 'is-invalid': errors.vulnerability_type }"
                  placeholder="Description"
                />
                <p v-if="errors.vulnerability_type" class="field-error">
                  {{ errors.vulnerability_type }}
                </p>
              </div>
              <div class="field-block">
                <label class="field-label" for="initial_education_level">Niveau initial</label
                ><input
                  id="initial_education_level"
                  v-model="form.initial_education_level"
                  type="text"
                  class="field-input"
                  :class="{ 'is-invalid': errors.initial_education_level }"
                  placeholder="Niveau d'etude"
                />
                <p v-if="errors.initial_education_level" class="field-error">
                  {{ errors.initial_education_level }}
                </p>
              </div>
              <div class="field-block field-block--full">
                <label class="field-label" for="current_status">Statut actuel</label
                ><input
                  id="current_status"
                  ...
                  placeholder="Statut ou situation actuelle"
                />
                <p v-if="errors.current_status" class="field-error">{{ errors.current_status }}</p>
              </div>
            </div>
          </section>
          ...
          <section class="form-section">
            <div class="form-section__header">
              <h2>Photo du beneficiaire</h2>
              <p>Ajoutez une image de profil avec previsualisation immediate.</p>
            </div>
              ...
              <div class="preview-card">
                <div class="preview-card__image">
                  <img
                    ...
                    class="photo-preview"
                  />
                  <div v-else class="photo-placeholder">Aucune photo selectionnee</div>
                </div>
                <div>
                  <p class="preview-card__title">Previsualisation</p>
                  <p class="preview-card__text">
                    L'image sera envoyee avec le formulaire en multipart/form-data.
                  </p>
                </div>
              </div>
            </div>
...
onMounted(async () => {
  ...
  try {
    ...
    formError.value =
      error.response?.data?.message || error.message || 'Unable to load beneficiary.'
...
function populateForm(beneficiary = {}) {
  ...
  form.current_country =
    beneficiary.current_country ||
    beneficiary.latest_academic_record?.school?.commune?.region?.country?.name ||
    ''
  form.current_city =
    beneficiary.current_city || beneficiary.latest_academic_record?.school?.commune?.name || ''
...
function handlePhotoChange(event) {
  ...
  errors.photo = ''
  if (!file) {
    if (!isEditing.value) setPhotoPreview('')
    return
  }
...
function validateForm() {
  ...
  if (form.type === 'alumni' && !form.exit_date)
    errors.exit_date = 'Exit date is required for alumni.'
...
function buildPayload() {
  ...
  const values = {
    ...
    vulnerability_type: form.vulnerability_type.trim(),
    initial_education_level: form.initial_education_level.trim(),
    current_status: form.current_status.trim(),
    gender: form.gender,
    type: form.type,
    date_of_birth: form.date_of_birth,
    entry_date: form.entry_date,
    exit_date: form.type === 'alumni' ? form.exit_date : '',
...
function normalizeApiErrors(validationErrors = {}) {
...
async function submitForm() {
  ...
  try {
    ...
    if (error.response?.status === 422 && error.response?.data?.errors) {
      normalizeApiErrors(error.response.data.errors)
      formError.value = 'Please correct the highlighted fields and try again.'
    } else
      formError.value =
        error.response?.data?.message || error.message || 'Unable to save beneficiary.'
...
.loading-card {
  ...
  background: #fff;
  box-shadow: 0 18px 44px rgba(4, 84, 128, 0.08);
}
.page-hero {
  display: flex;
...
.preview-card {
  ...
  background: linear-gradient(180deg, #fbfdff 0%, #f1f8fc 100%);
  padding: 1rem;
}
.preview-card__image {
  display: flex;
...
.loading-card__block,
.loading-card__line {
  background: linear-gradient(90deg, #e6f3fb, #f4f9fb, #e6f3fb);
...
.loading-card__line + .loading-card__line {
  margin-top: 0.65rem;
}
