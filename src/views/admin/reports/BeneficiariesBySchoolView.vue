  ...
  <div class="report-view">
    ...
    <header class="page-hero mb-4 d-flex justify-content-between align-items-start flex-wrap gap-3">
      <div>
        ...
        <h1 class="page-hero__title">Bénéficiaires par école</h1>
        <p class="page-hero__copy">
          Visualisez la répartition des bénéficiaires par établissement scolaire. Comparez les
    ...
    <div class="row g-4 mb-4">
      <div class="col-md-4">
        <div class="stats-card">
          <span class="stats-card__label">Total Écoles</span>
          <div v-if="loading" class="skel-wrap">
            <div class="skel skel-val"></div>
            <div class="skel skel-label"></div>
          </div>
          <template v-else>
            <strong class="stats-card__value">{{ totalSchoolsCount }}</strong>
            <small class="stats-card__sub text-muted">écoles actives enregistrées</small>
          </template>
        </div>
      </div>
      <div class="col-md-4">
        <div class="stats-card">
          ...
          <template v-else>
            ...
            <small class="stats-card__sub text-muted">effectif total scolaire cumulé</small>
          </template>
        </div>
      </div>
      <div class="col-md-4">
        <div class="stats-card">
          ...
          <template v-else>
            <div class="d-flex align-items-center">
              <strong class="stats-card__value text-orange">{{ highestSchoolName }}</strong>
              <span class="badge bg-orange count-badge ms-2" v-if="highestSchoolCount > 0">{{
                highestSchoolCount
              }}</span>
            </div>
            <small class="stats-card__sub text-muted"
              >établissement avec le plus grand effectif</small
            >
          </template>
        </div>
      </div>
      ...
      <div class="col-lg-7">
        <div class="panel-card p-4">
          <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-3">
            <div>
              <h2 class="panel-title">Répartition scolaire</h2>
              <p class="panel-copy mb-0">Recherchez et triez les données par école.</p>
            </div>
            <div class="d-flex gap-2">
              <input
                v-model="searchQuery"
              ...
              <select v-model="sortBy" class="form-select field-input" style="max-width: 180px">
                <option value="name_asc">École (A-Z)</option>
                <option value="name_desc">École (Z-A)</option>
          ...
          <div v-else-if="filteredSchools.length === 0" class="empty-state p-5 text-center">
            <span class="empty-icon">🏫</span>
            <p class="mb-0 text-muted">Aucune école trouvée avec ces critères.</p>
          </div>
          ...
          <div v-else>
            <div class="table-responsive">
              <table class="data-table">
                ...
                <tbody>
                  <tr v-for="school in paginatedSchools" :key="school.id" class="table-row">
                    <td>
                      <div class="d-flex align-items-center">
                        <span class="geo-icon me-2">🏫</span>
                        <strong>{{ school.name || 'Inconnu' }}</strong>
                      </div>
                    </td>
                    <td>
                      <span class="badge count-badge bg-orange">
                        {{ school.beneficiaries_count || 0 }}
                      </span>
                    </td>
                    <td>
                      <strong>{{ calculatePercentage(school.beneficiaries_count) }}%</strong>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2">
              <span class="small text-muted">
                ...
                {{ Math.min(startIndex + itemsPerPage, filteredSchools.length) }} sur
      ...
      <div class="col-lg-5">
        <div class="panel-card p-4 h-100 d-flex flex-column">
          <div class="mb-3">
            ...
            <p class="panel-copy">Représentation visuelle des écoles (Top 10)</p>
          </div>
...
async function loadData() {
  ...
  error.value = ''
  try {
    const data = await reportService.beneficiariesBySchool()
    schools.value = data || []
  } catch (err) {
    console.error(err)
    error.value = 'Erreur lors du chargement des données de rapport par école.'
...
const totalBeneficiariesCount = computed(() => {
  return schools.value.reduce((sum, s) => sum + (s.beneficiaries_count || 0), 0)
})

const highestSchool = computed(() => {
  if (schools.value.length === 0) return null
  return [...schools.value].sort(
    (a, b) => (b.beneficiaries_count || 0) - (a.beneficiaries_count || 0),
  )[0]
})

const highestSchoolName = computed(() => highestSchool.value?.name || '—')
const highestSchoolCount = computed(() => highestSchool.value?.beneficiaries_count || 0)
...
const paginatedSchools = computed(() => {
  return filteredSchools.value.slice(startIndex.value, startIndex.value + itemsPerPage.value)
})
...
function buildChart() {
  ...
  const sortedData = [...filteredSchools.value]
    .sort((a, b) => (b.beneficiaries_count || 0) - (a.beneficiaries_count || 0))
    .slice(0, 10)

  if (sortedData.length === 0) return

  const labels = sortedData.map((s) => s.name || 'Inconnu')
  const values = sortedData.map((s) => s.beneficiaries_count || 0)
...
watch([filteredSchools, chartCanvas], () => {
  nextTick(buildChart)
})
...
function buildExportRows() {
  return filteredSchools.value.map((item) => ({
    École: item.name || 'Inconnu',
...
function exportExcel() {
  ...
  downloadBlob(excelHtml, 'beneficiaires-par-ecole.xls', 'application/vnd.ms-excel;charset=utf-8;')
...
function exportPdf() {
  ...
  popup.document.write(`
    <html>
      <head>
        <title>Bénéficiaires par école</title>
        <style>
      ...
      <body>
        <h1>Répartition des bénéficiaires par école</h1>
