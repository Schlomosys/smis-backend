    ...
    <div class="page-shell">
      <header class="hero-banner">
        <div class="hero-banner__copy">
          <p class="hero-banner__eyebrow">Détails du bénéficiaire</p>
          <h1 class="hero-banner__title">{{ fullName }}</h1>
          <p class="hero-banner__text">
            Consultez le profil, les parrainages, les appuis, les informations academiques et
            l'historique familial.
          </p>
        </div>

        <div class="hero-banner__actions">
          <router-link to="/admin/beneficiaries" class="ghost-button"
            >Retour a la liste</router-link
          >
          <router-link
            v-if="beneficiary?.id"
      ...
      <template v-else-if="beneficiary">
        <section class="profile-card">
          <div class="profile-card__identity">
            <img
              v-if="beneficiary.photo_url"
              :src="beneficiary.photo_url"
              class="beneficiary-avatar beneficiary-avatar--xl"
            />
            <UserAvatar
              v-else
            ...
            <div class="profile-card__summary">
              <div class="badge-row">
                <span
                  :class="[
                    'status-badge',
                    beneficiary.type === 'alumni' ? 'status-badge--alumni' : 'status-badge--active',
                  ]"
                >
                  {{ beneficiary.type || 'non defini' }}
                </span>
                <span v-if="beneficiary.is_disabled" class="status-badge status-badge--highlight"
                  >Handicap</span
                >
              </div>
        ...
        <section class="overview-grid">
          <article class="detail-card">
            <div class="detail-card__header">
              ...
              <span class="detail-card__accent">Vue d'ensemble</span>
            </div>
            ...
            <dl class="detail-list">
              <div>
                <dt>Pays actuel</dt>
                <dd>
                  {{ beneficiary.current_country?.name || beneficiary.current_country || '-' }}
                </dd>
              </div>
              <div>
                <dt>Ville actuelle</dt>
                <dd>{{ beneficiary.current_city?.name || beneficiary.current_city || '-' }}</dd>
              </div>
              <div>
                <dt>Niveau initial</dt>
                <dd>{{ beneficiary.initial_education_level || '-' }}</dd>
              </div>
              <div>
                <dt>Date d'entree</dt>
                <dd>{{ formatDate(beneficiary.entry_date) }}</dd>
              </div>
              <div>
                <dt>Date de sortie</dt>
                <dd>{{ formatDate(beneficiary.exit_date) }}</dd>
              </div>
              <div>
                <dt>Date de naissance</dt>
                <dd>{{ formatDate(beneficiary.date_of_birth) }}</dd>
              </div>
            </dl>
          </article>
          ...
          <article class="detail-card">
            <div class="detail-card__header">
              <h3>Éducation Actuelle</h3>
              <span class="detail-card__accent">Scolarité</span>
            </div>

            <dl class="detail-list">
              <div>
                <dt>Année académique</dt>
                <dd>{{ beneficiary.latest_academic_record?.academic_year || '—' }}</dd>
              </div>
              <div>
                <dt>Classe</dt>
                <dd>{{ beneficiary.latest_academic_record?.class_name || '—' }}</dd>
              </div>
              <div>
                <dt>École</dt>
                <dd>{{ beneficiary.latest_academic_record?.school?.name || '—' }}</dd>
              </div>
              <div>
                <dt>Commune</dt>
                <dd>{{ beneficiary.latest_academic_record?.school?.commune?.name || '—' }}</dd>
              </div>
              <div>
                <dt>Pays</dt>
                <dd>
          ...
          <article class="detail-card detail-card--summary">
            <div class="detail-card__header">
              <h3>Résumé rapide</h3>
              <span class="detail-card__accent detail-card__accent--orange">Aperçu</span>
            </div>

            <p>
              {{ fullName }} est actuellement marque comme
              <strong>{{ beneficiary.type || 'profil non defini' }}</strong> avec un statut
              <strong>{{ beneficiary.current_status || 'non renseigne' }}</strong
              >.
            </p>
            <p>
              Cette zone met en avant les informations d'identification et de contact avant les
              actions detaillees.
            </p>
          </article>
        </section>
        ...
        <section class="tabs-card">
          <div class="tabs-card__header">
            ...
            <div class="quick-actions">
              <button type="button" class="ghost-button" @click="activeTab = 'academic-records'">
                Academique
              </button>
              <button type="button" class="ghost-button" @click="activeTab = 'supports'">
                Appuis
              </button>
              <button type="button" class="ghost-button" @click="activeTab = 'sponsorships'">
                Parrainages
              </button>
              <button type="button" class="ghost-button" @click="activeTab = 'family-history'">
                Famille
              </button>
            </div>
          </div>
          ...
          <div class="tab-content">
            <div v-if="activeTab === 'overview'" class="overview-placeholder">
              Les informations de synthèse sont affichées dans les cartes au-dessus.
            </div>
            <AcademicTab
...
const tabs = [
  ...
  { label: 'Historique scolaire', value: 'academic-records' },
  { label: 'Appuis', value: 'supports' },
  { label: 'Parrainages', value: 'sponsorships' },
  { label: 'Profil familial', value: 'family-history' },
]

const fullName = computed(() => {
  const parts = [beneficiary.value?.first_name, beneficiary.value?.last_name].filter(Boolean)
  return parts.length ? parts.join(' ') : 'Bénéficiaire'
})

const photoUrl = computed(() =>
  resolvePhotoUrl(
    beneficiary.value?.photo_url ||
      beneficiary.value?.photo ||
      beneficiary.value?.photo_path ||
      beneficiary.value?.image ||
      beneficiary.value?.image_url ||
      beneficiary.value?.avatar_url ||
      '',
  ),
)
...
const beneficiarySponsorships = computed(() =>
  normalizeCollection(beneficiary.value?.sponsorships || beneficiary.value?.sponsorship),
)
const families = computed(() =>
  normalizeCollection(
    referenceData.value.families ||
      referenceData.value.family_profiles ||
      referenceData.value.familyProfiles,
  ),
)
...
async function loadBeneficiary() {
  ...
  try {
    ...
    showToast({
      type: 'error',
      message: error.response?.data?.message || 'Unable to load beneficiary.',
    })
  } finally {
    loading.value = false
  }
...
function resolvePhotoUrl(value) {
  ...
  if (apiBase) {
    ...
    if (normalizedPath.startsWith('/public/'))
      return `${apiBase}${normalizedPath.replace(/^\/public/i, '/storage')}`
...
.hero-banner {
  ...
  gap: 1.25rem;
  flex-wrap: wrap;
  background: linear-gradient(135deg, #02334d 0%, #045480 60%, #006fb8 100%);
  color: #fff;
  padding: 1.6rem;
}
...
.status-badge--active {
  background: rgba(77, 163, 217, 0.18);
  color: #045480;
}
.status-badge--alumni {
...
.state-card__skeleton {
  ...
  background: linear-gradient(90deg, #e6f3fb, #f3f8fb, #e6f3fb);
...
.state-card__line + .state-card__line {
  margin-top: 0.65rem;
}
.state-card__line--short {
  width: 55%;
}
@keyframes shimmer {
  0% {
